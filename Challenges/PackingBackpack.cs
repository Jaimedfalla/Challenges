...
public class PackingBackpack : IAlgorithm
{
    public void Execute()
    {
        ...
        for (int i = 1; i <= n; i++)
        {
            ...
            {
                string variables = $"({i},{w}) -> weights[i - 1]={weights[i - 1]}";
                string ifText = string.Empty;
                string elseText = string.Empty;

                if (weights[i - 1] <= w)
                {
                    dp[i, w] = Math.Max(scores[i - 1] + dp[i - 1, w - weights[i - 1]], dp[i - 1, w]);
                    ifText = $"if({weights[i - 1]} <= {w}) =======> Math.Max({scores[i - 1]} + {dp[i-1,w-weights[i - 1]]}, {dp[i - 1, w]})={Math.Max(scores[i - 1] + dp[i - 1, w - weights[i - 1]], dp[i - 1, w])}";
                }
                else
                {
                    dp[i, w] = dp[i - 1, w];
                    elseText = $"else -> dp[i - 1, w]={dp[i - 1, w]}";
                }
                Console.WriteLine($"{variables}, {ifText}, {elseText}");
                fila.Append($"{dp[i,w]},");
            }
